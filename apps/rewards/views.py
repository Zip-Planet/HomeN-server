...
class RewardDetailView(APIView):
    ...
    @extend_schema(
        ...
        description=(
            ...
            "1등부터 순위를 매긴다.\n\n"
            "받기 완료 화면의 **수령자·수령 일시**는 목록 항목과 동일한 `claim` 객체로 내려간다 "
            "(미수령이면 null).\n\n"
            "## 🔐 인증\n"
            "Bearer access 토큰 필수.\n\n"
            "## 📥 요청\n"
            ...
            "| 위치 | 필드 | 타입 | 설명 |\n"
            "| --- | --- | --- | --- |\n"
            "| body | `status` | string | claimed / claimable / in_progress |\n"
            "| body | `claim` | object\\|null | 수령 이력. 미수령이면 null |\n"
            "| body | `claim.claimed_by` | object\\|null | 수령자 {uid, name, profile_image}. 탈퇴 시 null |\n"
            "| body | `claim.claimed_point` | integer | 수령 시점 목표 포인트 스냅샷 |\n"
            "| body | `claim.claimed_at` | datetime | 수령 일시 |\n"
            "| body | `member_progress[].rank` | integer | 보유 포인트 기준 순위 |\n"
            "| body | `member_progress[].uid` | string | 구성원 uid |\n"
            "| body | `member_progress[].name` | string | 구성원 닉네임 |\n"
            "| body | `member_progress[].profile_image` | integer\\|null | 프로필 이미지 enum |\n"

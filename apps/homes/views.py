...
class HomeDashboardView(APIView):
    ...
    @extend_schema(
        ...
        description=(
            ...
            "- **리포트 카드 기여도(`contribution`)** = 이번 주 집 전체 완료 포인트가 0 이면 지난주 기여도로 대체"
            "(`is_last_week: true` → '지난주 기여도 N%'), 지난주도 0 이면 null → 화면 '없음'\n"
            ...
            "| body | `contribution` | object | `{rate, week_start, is_last_week}` 또는 null(없음) |\n"

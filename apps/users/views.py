...
class KakaoLoginView(APIView):
    ...
    @extend_schema(
        ...
        description=(
            "## 🔥 설명\n"
            "카카오 OAuth2 인가 코드(`code`)로 access/refresh JWT 를 발급한다. 신규 유저는 자동 가입되며 "
            "재로그인은 `provider_id` 로 매칭된다.\n\n"
            "## 📥 요청\n"
            "| 위치 | 필드 | 타입 | 필수 | 설명 |\n"
            "| --- | --- | --- | --- | --- |\n"
            ...
            "| 위치 | 필드 | 타입 | 설명 |\n"
            "| --- | --- | --- | --- |\n"
            "| body | `access` | string | JWT 액세스 토큰 (기본 1시간) |\n"
            "| body | `refresh` | string | JWT 리프레시 토큰 (기본 7일) |\n"
            "| body | `is_profile_set` | boolean | 닉네임+이미지 모두 설정됐는지 여부 (false면 온보딩) |\n"
            "| body | `has_home` | boolean | 집 관리자/구성원으로 소속됐는지 여부 |\n\n"
            "## ❌ 에러\n"
            "| status | code | 의미 |\n"
            "| --- | --- | --- |\n"
            ...
            "**요청:**\n"
            "```bash\n"
            "curl -X POST '{host}/api/v1/auth/kakao/' \\\n"
            "     -H 'Content-Type: application/json' \\\n"
            "     -d '{\"code\":\"abc123XYZ_kakao_authorization_code\"}'\n"
            "```\n\n"
            "**응답 (200):**\n"
            "```json\n"
            "{\n"
...
class AppleLoginView(APIView):
    ...
    @extend_schema(
        ...
        description=(
            "## 🔥 설명\n"
            "Apple Sign In 인가 코드(`code`)로 access/refresh JWT 를 발급한다. Apple `refresh_token` 은 "
            "탈퇴 시 revocation 을 위해 `SocialAccount.refresh_token` 에 저장한다.\n\n"
            "## 📥 요청\n"
            "| 위치 | 필드 | 타입 | 필수 | 설명 |\n"
            "| --- | --- | --- | --- | --- |\n"
            ...
            "| body | `access` | string | JWT 액세스 토큰 (기본 1시간) |\n"
            "| body | `refresh` | string | JWT 리프레시 토큰 (기본 7일) |\n"
            "| body | `is_profile_set` | boolean | 닉네임+이미지 모두 설정됐는지 여부 |\n"
            "| body | `has_home` | boolean | 집 관리자/구성원으로 소속됐는지 여부 |\n\n"
            "## ❌ 에러\n"
            "| status | code | 의미 |\n"
            "| --- | --- | --- |\n"
            ...
            "**요청:**\n"
            "```bash\n"
            "curl -X POST '{host}/api/v1/auth/apple/' \\\n"
            "     -H 'Content-Type: application/json' \\\n"
            "     -d '{\"code\":\"c1.0.abcdef.0.apple_authorization_code\"}'\n"
            "```\n\n"
            "**응답 (200):**\n"
            "```json\n"
            "{\n"
...
class LogoutView(APIView):
    ...
    @extend_schema(
        ...
        description=(
            "## 🔥 설명\n"
            "보유 중인 refresh 토큰을 SimpleJWT 블랙리스트에 등록해 추가 갱신을 차단한다. "
            "access 토큰은 자체 무효화 없이 만료까지 유효하다.\n\n"
            "## 🔐 인증\n"
            "Bearer access 토큰 필수 (`Authorization: Bearer <access>`).\n\n"
            "## 📥 요청\n"
            "| 위치 | 필드 | 타입 | 필수 | 설명 |\n"
            "| --- | --- | --- | --- | --- |\n"
            ...
            "**요청:**\n"
            "```bash\n"
            "curl -X POST '{host}/api/v1/auth/logout/' \\\n"
...
class UserMeView(APIView):
    ...
    @extend_schema(
        ...
        description=(
            ...
            "`profile_image`/`home_role` 이 null 일 수 있다.\n\n"
            "## 🔐 인증\n"
            "Bearer access 토큰 필수.\n\n"
            "## 📥 요청\n"
            "요청 본문 없음.\n\n"
            ...
            "| 위치 | 필드 | 타입 | 설명 |\n"
            "| --- | --- | --- | --- |\n"
            "| body | `uid` | string (uuid) | 유저 고유 식별자 |\n"
            "| body | `name` | string | 닉네임 (1~8자, 미설정 시 빈 문자열) |\n"
            "| body | `profile_image` | integer\\|null | 프로필 이미지 enum (1~8, 미설정 시 null) |\n"
            "| body | `is_profile_set` | boolean | 닉네임+이미지 모두 설정 여부 |\n"
            "| body | `has_home` | boolean | 집 소속 여부 |\n"
            "| body | `home_role` | integer\\|null | 1=관리자, 2=구성원, 집 없으면 null |\n\n"
            "## ❌ 에러\n"
            "| status | code | 의미 |\n"
            "| --- | --- | --- |\n"
            "| 401 | `authentication_failed` | access 토큰 누락/만료 |\n\n"
            ...
            "**요청:**\n"
            "```bash\n"
            "curl -X GET '{host}/api/v1/users/me/' \\\n"
            "     -H 'Authorization: Bearer <access>'\n"
            "```\n\n"
            "**응답 (200):**\n"
            "```json\n"
    ...
    @extend_schema(
        ...
        description=(
            "## 🔥 설명\n"
            "닉네임과 프로필 이미지를 함께 갱신한다. 온보딩(최초 설정)과 변경 모두 동일 엔드포인트를 사용한다.\n\n"
            "## 🔐 인증\n"
            "Bearer access 토큰 필수.\n\n"
            "## 📥 요청\n"
            "| 위치 | 필드 | 타입 | 필수 | 설명 |\n"
            ...
            "| body | `profile_image` | integer | ✓ | 프로필 이미지 enum (1~8) |\n\n"
            "## 📤 응답 (200)\n"
            "변경 후 최신 프로필 (필드는 `GET /users/me/` 와 동일).\n\n"
            "## ❌ 에러\n"
            "| status | code | 의미 |\n"
            "| --- | --- | --- |\n"
            ...
            "**요청:**\n"
            "```bash\n"
            "curl -X PATCH '{host}/api/v1/users/me/' \\\n"
            "     -H 'Authorization: Bearer <access>' \\\n"
            "     -H 'Content-Type: application/json' \\\n"
            "     -d '{\"name\":\"홍길동\",\"profile_image\":3}'\n"
            "```\n\n"
            "**응답 (200):**\n"
            "```json\n"
            "{\n"
    ...
    @extend_schema(
        ...
        description=(
            "## 🔥 설명\n"
            "현재 유저를 탈퇴 처리한다. 집 관리자는 직접 탈퇴할 수 없고 양도/집 삭제가 선결되어야 한다. "
            "Apple 가입자는 저장된 refresh_token 으로 token revocation 까지 처리한다.\n\n"
            "## 🔐 인증\n"
            "Bearer access 토큰 필수.\n\n"
            "## 📥 요청\n"
            "요청 본문 없음.\n\n"
            ...
            "**요청:**\n"
            "```bash\n"
            "curl -X DELETE '{host}/api/v1/users/me/' \\\n"
            "     -H 'Authorization: Bearer <access>'\n"
            "```\n"
        ),
        responses={
...
class ProfileImageListView(APIView):
    ...
    @extend_schema(
        ...
        description=(
            "## 🔥 설명\n"
            "선택 가능한 프로필 이미지 enum 정수 목록을 반환한다. 인증 없이 호출 가능.\n"
            ...
            "| 위치 | 필드 | 타입 | 설명 |\n"
            "| --- | --- | --- | --- |\n"
            "| body[*] | `id` | integer | 프로필 이미지 enum ID (1~5) |\n\n"
            "## 💻 예제\n"
            "**요청:**\n"
            "```bash\n"
            "curl -X GET '{host}/api/v1/users/profile-images/'\n"
            "```\n\n"
            "**응답 (200):**\n"
            "```json\n"
...
class NicknameAvailabilityView(APIView):
    ...
    @extend_schema(
        ...
        description=(
            "## 🔥 설명\n"
            "주어진 닉네임을 다른 유저가 점유 중인지 확인한다. **존재 여부만** 확인하며, 형식 검증은 "
            "`PATCH /users/me/` 시점에 수행된다.\n\n"
            "## 🔐 인증\n"
            "Bearer access 토큰 필수.\n\n"
            "## 📥 요청\n"
            "| 위치 | 필드 | 타입 | 필수 | 설명 |\n"
            ...
            "| 위치 | 필드 | 타입 | 설명 |\n"
            "| --- | --- | --- | --- |\n"
            "| body | `is_available` | boolean | true=사용 가능, false=이미 사용 중 |\n\n"
            "## ❌ 에러\n"
            "| status | code | 의미 |\n"
            "| --- | --- | --- |\n"
            "| 401 | `authentication_failed` | access 토큰 누락/만료 |\n\n"
            ...
            "**요청:**\n"
            "```bash\n"
            "curl -X GET '{host}/api/v1/users/nicknames/홍길동/' \\\n"
            "     -H 'Authorization: Bearer <access>'\n"
            "```\n\n"
            "**응답 (200):**\n"
            "```json\n"
